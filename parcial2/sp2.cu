...
void cudamult(int f1, int c1, int c2, double* M1, double* M2, double* M3){
        double *d_M, *d_N, *d_P;
        cudaMalloc(&d_M, sizeof(double) * f1 * c1);
        cudaMalloc(&d_N, sizeof(double) * c1 * c2);
        cudaMalloc(&d_P, sizeof(double) * f1 * c2);
        ...
        error = cudaMemcpy(d_N, M2, c1 * c2 * sizeof(double), cudaMemcpyHostToDevice);
        if(error != cudaSuccess){
        ...
        dim3 dimGrid(ceil(f1/double(blockSize)),ceil(c2/double(blockSize)),1);
        matrixMulKernel<<<dimGrid,dimBlock>>>(d_M,d_N,d_P, f1, c1, c2);
        cudaDeviceSynchronize();
        cudaMemcpy(M3,d_P, f1 * c2 * sizeof(double),cudaMemcpyDeviceToHost);
        cudaFree(d_M);
        cudaFree(d_N);
        cudaFree(d_P);
